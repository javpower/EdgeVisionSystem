...
public class CameraSourceFactory {
        ...
        if (src instanceof Integer) {
            ...
            if (strSrc.startsWith(MVS_IP_PREFIX)) {
                ...
                if (isValidIpAddress(ipAddress)) {
                    return createMVSCameraSource(ipAddress);
                } else {
     ...
     */
    private static CameraSource createMVSCameraSource(String ipAddress) {
        ...
        if (!mvsAvailable) {
            ...
            throw new IllegalArgumentException("MVS SDK not available. Cannot create MVS camera source: " + ipAddress);
        }

        try {
            // 使用反射创建 MVSCameraSource 实例
            Class<?> mvsClass = Class.forName(MVS_CAMERA_CLASS_NAME);
            return (CameraSource) mvsClass.getConstructor(String.class).newInstance(ipAddress);
        } catch (Exception e) {
